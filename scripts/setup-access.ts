 ...
 * 
 * Sets up Cloudflare Access to protect your OWOKX worker endpoints.
 ...
 *   OWOKX_WORKER_URL - Your worker URL (e.g., https://OWOKX.your-subdomain.workers.dev)
 * 
 * Optional:
 *   OWOKX_ALLOWED_EMAILS - Comma-separated list of allowed emails
 * 
 * Usage:
 *   npx tsx scripts/setup-access.ts
 * 
 * Or with environment variables inline:
 *   CLOUDFLARE_API_TOKEN=xxx CLOUDFLARE_ACCOUNT_ID=yyy OWOKX_WORKER_URL=https://... npx tsx scripts/setup-access.ts
 */
...
async function main() {
  console.log("\n🔐 OWOKX Cloudflare Access Setup\n");

  const accountId = process.env.CLOUDFLARE_ACCOUNT_ID;
  const workerUrl = process.env.OWOKX_WORKER_URL;
  const allowedEmails = process.env.OWOKX_ALLOWED_EMAILS?.split(",").map((e) => e.trim()).filter(Boolean) || [];
  ...
  if (!workerUrl) {
    console.error("❌ OWOKX_WORKER_URL is required");
    console.error("   Example: https://OWOKX.your-subdomain.workers.dev");
    process.exit(1);
  }
  ...
  const appName = "OWOKX Trading Agent";
  ...
  console.log("Your OWOKX endpoints are now protected.");
  ...
  if (allowedEmails.length === 0) {
    ...
    console.log("   OWOKX_ALLOWED_EMAILS=you@example.com,team@example.com npx tsx scripts/setup-access.ts");
  } else {
