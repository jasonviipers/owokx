...
export function SettingsModal({ config, onSave, onClose, onReset }: SettingsModalProps) {
  ...
  const handleBrokerChange = (broker: 'alpaca' | 'okx' | 'polymarket') => {
    ...
    setLocalConfig(prev => {
      ...
      if (broker === 'okx') {
        const base = prev.crypto_symbols && prev.crypto_symbols.length > 0
          ? prev.crypto_symbols
          : ['BTC/USD', 'ETH/USD', 'SOL/USD']
        next.crypto_symbols = base.map(s => s.toUpperCase().replace(/\/USD$/, '/USDT'))
      }
      return next
    })
  }
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={onClose}>
      ...
      >
        <div onClick={e => e.stopPropagation()} className="space-y-6">
          ...
          <div>
            <h3 className="hud-label mb-3 text-hud-primary">Broker</h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="hud-label block mb-1">Trading Broker</label>
                <select
                  ...
                  {localConfig.llm_provider === 'ai-sdk' && (
                    <>
                      <optgroup label="OpenAI">
                        <option value="openai/gpt-5.2-2025-12-11">GPT-5.2 (best)</option>
