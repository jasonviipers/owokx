...
export class PolicyEngine {
  ...
  private checkDailyLossLimit(ctx: PolicyContext, violations: PolicyViolation[]): void {
    if (!Number.isFinite(ctx.account.equity) || ctx.account.equity <= 0) {
      violations.push({
        rule: "account_equity_invalid",
        message: "Account equity is invalid; cannot evaluate daily loss limit safely",
        current_value: ctx.account.equity,
        limit_value: "> 0",
      });
      return;
    }
